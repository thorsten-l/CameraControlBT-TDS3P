...
// TFT Pin check
#if BOARD_SPI_MOSI != TFT_MOSI || BOARD_SPI_SCK != TFT_SCLK ||                 \
    BOARD_TFT_CS != TFT_CS || BOARD_TFT_DC != TFT_DC ||                        \
    BOARD_TFT_RST != TFT_RST || TFT_WIDTH != BOARD_TFT_WIDTH ||                \
    TFT_HEIGHT != BOARD_TFT_HEIHT
#error                                                                         \
    "Error! Please make sure <Setup214_LilyGo_T_Display_S3_Pro.h> is selected in <TFT_eSPI/User_Setup_Select.h>"
#error                                                                         \
    "Error! Please make sure <Setup214_LilyGo_T_Display_S3_Pro.h> is selected in <TFT_eSPI/User_Setup_Select.h>"
#error                                                                         \
    "Error! Please make sure <Setup214_LilyGo_T_Display_S3_Pro.h> is selected in <TFT_eSPI/User_Setup_Select.h>"
...
void lv_helper(uint8_t r)
{
  ...
  if (!hasTouch)
  {
    ALOG_E("Failed to find Capacitive Touch !");
  }
  else
  {
    ALOG_I("Find Capacitive Touch");
